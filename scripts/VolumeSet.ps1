...
process
    {   Invoke-DSCCAutoReconnect
        ...
        switch ( $devicetype )
            {   'device-type1'  {   $MyURI = $BaseURI + 'storage-systems/' + $DeviceType + '/' + $SystemId + '/applicationsets'
                                    if ( $VolumeSetId )
                                            {   $MyURI = $MyURI + '/' + $VolumeSetId
                                            }
                                    $SysColOnly = @()  
                                    if ( $WhatIf )
                                            ...
                                            {   try     {   $MyCol = invoke-restmethod -uri $MyUri -headers $MyHeaders -method Get    
                                                        }
                                                catch   {   $MyCol = @{ total = 0 }
                                                        }
                                            }
                                    ...
                                    if ( ( $MyCol ).total -eq 0 )
                                            {   Write-Warning "The Call to SystemID $SystemId returned no Application Sets."
                                                $MyCol = ''                                                
                                            }  
                                        else
                                            {   $SysColOnly += $MyCol                                                        
                                            }
                                    $ReturnData = Invoke-RepackageObjectWithType -RawObject $SysColOnly -ObjectName ( "VolumeSet")
                                    return $ReturnData
                                }
                'device-type2'  {   $MyURI = $BaseURI + 'storage-systems/' + $DeviceType + '/' + $SystemId + '/volume-collections' 
                                    if ( $VolumeSetId )
                                        {   $MyURI = $MyURI + '/' + $VolumeSetId
                                        }
                                    $SysColOnly = @() 
                                    ...
                                    if ( ( $SysColOnly ).total -eq 0 )
                                            {   Write-Warning "The Call to SystemID $SystemId returned Volume Collections."
                                                return                                                
                                            }
                                    $ReturnData = Invoke-RepackageObjectWithType -RawObject $SysColOnly -ObjectName "VolumeSet"
                                    return $ReturnData          
...
process
    {   Invoke-DSCCAutoReconnect
        ...
        write-verbose "Dectected the DeviceType is $DeviceType"
        switch ( $devicetype )
            {   'device-type1'  {   $MyURI = $BaseURI + 'storage-systems/' + $DeviceType + '/' + $SystemId + '/applicationsets' + '/' + $VolumeSetId
                                    $SysColOnly = @()  
                                    if ( $WhatIf )
                                            {   $MyCol = invoke-restmethodWhatIf -uri $MyUri -headers $MyHeaders -method Delete
                                            }   
                                        else 
                                            {   try     {   $MyCol = invoke-restmethod -uri $MyUri -headers $MyHeaders -method Delete
                                                        }
                                                catch   {    Write-Warning "The Call to SystemID $SystemId to remove the Application Set failed."
                                                            return
                                                        }
                                            }
                                    return $MyCol
                                }
                'device-type2'  {   $MyURI = $BaseURI + 'storage-systems/' + $DeviceType + '/' + $SystemId + '/volume-collections' + '/' + $VolumeSetId
                                    $SysColOnly = @() 
                                    if ( $WhatIf )
                                            {   $SysColOnly = invoke-restmethodWhatIf -uri $MyUri -headers $MyHeaders -method Get
                                            }   
                                            ...
                                            {   try     {   $MyCol = invoke-restmethod -uri $MyUri -headers $MyHeaders -method Delete
                                                ...
                                                catch   {   Write-Warning "The Call to SystemID $SystemId to remove the Volume Collection failed."
                                                            return
                                                        }
                                            }
                                    return $MyCol          
...
#>   
[CmdletBinding()]
param(  [Parameter(ValueFromPipeLineByPropertyName=$true,Mandatory=$true,ParameterSetName=('device-type1') )]
        [Parameter(ValueFromPipeLineByPropertyName=$true,Mandatory=$true,ParameterSetName=('device-type2') )] 
                                                                                                [Alias('id')]   [string]    $SystemId,
        [Parameter(ParameterSetName=('device-type1') )]                                                         [string]    $appSetBuisnessUnit,
        [Parameter(ParameterSetName=('device-type1') )]                                                         [string]    $appSetComments,
        [Parameter(ParameterSetName=('device-type1') )]                                                         [string]    $appSetImportance,
        [Parameter(Mandatory=$true,ParameterSetName=('device-type1') )]                                         [string]    $appSetName,
        ...
        [Parameter(ParameterSetName=('device-type2') )]                                                         [string]    $description,
        [Parameter(ParameterSetName=('device-type2') )]                                                         [boolean]   $isStandaloneVolColl,
        [Parameter(Mandatory=$true,ParameterSetName=('device-type2') )]                                         [string]    $name,
    ...
    {   Invoke-DSCCAutoReconnect
        ...
        switch ( $devicetype )
            {   'device-type1'  {   $MyURI = $BaseURI + 'storage-systems/' + $devicetype + '/' + $SystemId + '/applicationsets'
                                    ...
                                    if ($appSetComments)                {   $MyBody += @{ appSetComments = $appSetComments }  }
                                    if ($appSetImportance )             {   $MyBody += @{ appSetImportance = $appSetImportance }  }
                                                                            $MyBody += @{ appSetName = $appSetName }  
